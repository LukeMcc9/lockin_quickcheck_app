...
WEIGHTS = {
    "PTS": 1.0, "REB": 1.0, "AST": 1.4, "STL": 2.4, "BLK": 2.0,
    "TOV": -2.0,
    "DD2": 2.0, "TD3": 4.0,
    "FGM": 1.0, "MISS_FG": -0.5, "FTM": 0.5, "MISS_FT": -0.1,
    "FG3M": 0.5, "MISS_3": -0.1, "OREB": 0.5,
}
...
def safe_league_log(season: str, stype: str = "Regular Season", pause: float = 0.7, retries: int = 3) -> pd.DataFrame:
    last_err = None
...
def compute_fp(df: pd.DataFrame) -> pd.Series:
    ...
    fp = (
        df["PTS"]*WEIGHTS["PTS"] + df["REB"]*WEIGHTS["REB"] + df["AST"]*WEIGHTS["AST"]
      + df["STL"]*WEIGHTS["STL"] + df["BLK"]*WEIGHTS["BLK"] + df["TOV"]*WEIGHTS["TOV"]
      + df["DD2"]*WEIGHTS["DD2"] + df["TD3"]*WEIGHTS["TD3"]
      + df["FGM"]*WEIGHTS["FGM"] + miss_fg*WEIGHTS["MISS_FG"]
      + df["FTM"]*WEIGHTS["FTM"] + miss_ft*WEIGHTS["MISS_FT"]
      + df["FG3M"]*WEIGHTS["FG3M"] + miss_3*WEIGHTS["MISS_3"]
...
def main():
    ...
    ap.add_argument("--season", required=True, help="Target season like 2025-26 (we will pull the prior two seasons).")
    ap.add_argument("--out", required=True, help="Output CSV path, e.g., data/lockin_baseline_2023-24_2024-25.csv")
    ...
    keep = [
        "SEASON","GAME_DATE","PLAYER_ID","PLAYER_NAME","TEAM_ABBREVIATION",
        "PTS","REB","AST","STL","BLK","TOV","FGM","FGA","FG3M","FG3A","FTM","FTA","OREB","DREB",
        "DD2","TD3","FP"
    ]
